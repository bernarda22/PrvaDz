...
namespace Game1
{
    ...
    public class Game1 : Game
    {
        ...
        /// oundEffect is a type defined in Monogame framework 
        /// </summary> 
        ...
        protected override void Initialize()
        {
            ...
            PaddleBottom.X = (screenBounds.Width - GameConstants.PaddleDefaultWidth) / 2;
            PaddleBottom.Y = screenBounds.Height - GameConstants.PaddleDefaulHeight;

            PaddleTop = new Paddle(GameConstants.PaddleDefaultWidth, GameConstants.PaddleDefaulHeight, GameConstants.PaddleDefaulSpeed);
            PaddleTop.X = (screenBounds.Width - GameConstants.PaddleDefaultWidth) / 2;
            PaddleTop.Y = 0;
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            {
                MoveLeft(PaddleBottom, gameTime);
            }

            if (touchState.IsKeyDown(Keys.Right))
            {
                MoveRight(PaddleBottom, gameTime);
            }

            if (touchState.IsKeyDown(Keys.A))
            {
                MoveLeft(PaddleTop, gameTime);
            }

            if (touchState.IsKeyDown(Keys.D))
            {
                MoveRight(PaddleTop, gameTime);
            }
        ...
        private void MoveLeft(Paddle paddle, GameTime gameTime)
        {
            float newOffset = paddle.X - (float)(paddle.Speed * gameTime.ElapsedGameTime.TotalMilliseconds);
            paddle.X = MathHelper.Clamp(newOffset, GraphicsDevice.Viewport.Bounds.Left, GraphicsDevice.Viewport.Bounds.Right - paddle.Width);
        }

        private void MoveRight(Paddle paddle, GameTime gameTime)
        {
            float newOffset = paddle.X + (float)(paddle.Speed * gameTime.ElapsedGameTime.TotalMilliseconds);
            paddle.X = MathHelper.Clamp(newOffset, GraphicsDevice.Viewport.Bounds.Left, GraphicsDevice.Viewport.Bounds.Right - paddle.Width);
        }
